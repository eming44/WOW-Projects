...
namespace Exercise1
{
    public class Character
    {
        ...
        public FactionRaceGender FactionRaceGenderGet
        {
            get
            {
                return this.factionRaceGender;
            }
        }
        ...
        {
            attacked.health -= damage + valueByLevel(attacker.level);
        ...
        public void castSpell(Character attacker, Character attacked, Spells spell)
        {
            if (attacker.characterClass.CharacterClass.Equals(spell.SpellClass.CharacterClass))
            {
                if (mana >= spell.ManaGet)
                {
                    ...
                    {
                        attacked.health -= spell.SpellValue + valueByLevel(attacker.level) + weapon.ItemValueGetSet;
                        attacked.health += attacked.armor;
                    ...
                    {
                        attacker.health += spell.SpellValue + valueByLevel(attacker.level) + weapon.ItemValueGetSet;
                    ...
                    {
                        attacked.health -= (spell.SpellValue - attacked.armor) + valueByLevel(attacker.level) + weapon.ItemValueGetSet;
                        attacked.armor -= 5;
                        attacker.health += spell.SpellValue + valueByLevel(attacker.level) + weapon.ItemValueGetSet;
                        if (attacker.health > attacker.healthCap)
                        {
                    ...
                    {
                        attacker.armor += spell.SpellValue + valueByLevel(attacker.level);
                    }
